...
async def run_interview_sync():
    ...
    hr_q = await hr_agent.invoke_async("応募者に最初の質問をしてください。")
    hr_q_txt = extract_text(hr_q)
    print(f"\n[HR] {hr_q_txt}")

    app_r = await applicant_agent.invoke_async(hr_q_txt)
    ...
    app_f = await applicant_agent.invoke_async(hr_f_txt)
    ...
    final_a = await applicant_agent.invoke_async(dept_q_txt)
